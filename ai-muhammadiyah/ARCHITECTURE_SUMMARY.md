...
| `<TasksPanel>` | ✅ `components/TasksPanel.tsx` + `TaskRow.tsx` (baru, langkah 21) | Isi asli tab Tasks. `useTasks()` juga dipanggil di dalam komponen sendiri, sama pola `DocsPanel`. **Penting:** tabel `tasks` = 1 row per LIST (jsonb array item), bukan 1 row per task — v1 sembunyikan konsep multi-list, selalu pakai row pertama sebagai "the" list per workspace. Reuse guard `workspaceId` falsy yang sama dengan `DocsPanel`. Lihat `MIGRATION_PROGRESS.md` langkah 21 untuk detail skema + keputusan kuota. |
...
| `tasks_ai_muhammadiyah_workspace_standardized/` | tab Tasks (di dalam SPA, bukan route) | `lib/tasks.ts`, `hooks/useTasks.ts`, `components/TasksPanel.tsx` | **SELESAI (langkah 21).** Struktur v1 = list flat + status per row (bukan kanban drag-drop dari mockup — butuh library baru, ditunda). Generate-dari-chat mengekstrak N action item sekaligus (beda dari Docs yang 1 klik = 1 dokumen), kuota per-item bukan per-klik. |
| `sheets_...`, `canvas_...` (2 tools sisa) | `/workspace/[id]/sheets` dst | **Belum ada hook** (tabel DB `sheets`/`canvases` sudah ada dari migration yang sama seperti `docs`/`tasks`, tapi belum dipakai kode manapun) | Di luar scope dokumen ini — fitur baru, bukan refactor. |
