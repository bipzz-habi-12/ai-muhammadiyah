...
| `<TopBar>` | 863-1139 | `<header>` — logo mobile, dropdown model AI (877-968ish), dropdown study mode/skill (900-1070ish), account menu popover kanan atas (1070-1139, **duplikat** dari sidebar). |
...
|---|---|---|---|
| `workspace_chat_ai_muhammadiyah_standardized/` | `/workspace/[id]` — halaman chat utama | **Semua** hook (semua 11) + hampir semua JSX di bagian 3 | **Gap arsitektur besar** (sebagian besar masih terbuka): Stitch pakai layout 4-panel (icon rail 60px + workspace-list sidebar 260px + chat area + knowledge sidebar kanan 240px `hidden lg:flex`). Kode saat ini cuma 1 sidebar gabungan (340px, **sudah direstyle** ke tema hijau tua Stitch di `components/Sidebar.tsx` — lihat `MIGRATION_PROGRESS.md` langkah 13) + chat area, **masih belum ada** icon rail terpisah atau knowledge sidebar kanan. Restyle Sidebar yang sudah dilakukan sengaja **tidak** mengadopsi split 4-panel ini — itu tetap perlu keputusan struktural terpisah (component baru, bukan cuma ganti className) sebelum dikerjakan. |
