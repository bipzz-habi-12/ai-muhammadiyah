...
\restrict K4gzEbHAC3U3ZPRvJTek26BpUx8TXs2FWcq78XYTy2VPOdtzeVKGtdFQZHOOhhj
...
--
-- Name: skill_change_log; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.skill_change_log (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    workspace_id uuid NOT NULL,
    skill_id uuid,
...
ALTER TABLE public.skill_change_log OWNER TO postgres;
...
CREATE TABLE public.skills (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT skills_category_check CHECK ((char_length(category) <= 120)),
    CONSTRAINT skills_name_check CHECK ((length(btrim(name)) > 0)),
    CONSTRAINT skills_owner_is_custom_match CHECK ((((owner_id IS NULL) AND (is_custom = false)) OR ((owner_id IS NOT NULL) AND (is_custom = true))))
);
...
COPY public.canvases (id, workspace_id, title, nodes, source_ref, created_at, updated_at) FROM stdin;
\.
...
COPY public.chat_workspaces (id, user_id, name, created_at, updated_at, icon, color, persona_config, skill_id) FROM stdin;
ca1a5d55-b78f-43c7-bda7-d01bfdd052a8	e6eb571b-71a6-467c-9f69-f47f1e0664f1	Habi	2026-06-02 02:51:44.745194+00	2026-06-02 02:51:44.745194+00	\N	\N	{}	\N
\.
...
COPY public.docs (id, workspace_id, title, content, source_ref, created_at, updated_at) FROM stdin;
\.
...
COPY public.sheets (id, workspace_id, title, data, source_ref, created_at, updated_at) FROM stdin;
\.


--
-- Data for Name: skill_change_log; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.skill_change_log (id, workspace_id, skill_id, conversation_id, changed_at) FROM stdin;
\.
...
COPY public.skills (id, owner_id, name, category, system_prompt, is_custom, created_at, updated_at) FROM stdin;
\.
...
f16104d1-687a-48c1-b908-f97a93243df0	b01e9a49-58e3-4efe-88e5-72e93a37f622	free	active	2026-07-04 10:22:33.52572+00	2026-07-04 10:22:33.52572+00	\N	2026-07-04 10:22:33.52572+00	2026-07-04 10:22:33.52572+00
af0a8df3-cb3e-42d7-876f-f91cc3b55e65	fa930b99-a432-407c-b2bc-d12e0df7033f	free	active	2026-07-04 10:22:33.52572+00	2026-07-04 10:22:33.52572+00	\N	2026-07-04 10:22:33.52572+00	2026-07-04 10:22:33.52572+00
\.
...
COPY public.tasks (id, workspace_id, title, tasks, source_ref, created_at, updated_at) FROM stdin;
\.
...
20260530000000	{"-- AI Muhammadiyah persistent chat history.\r\n-- Run this in the Supabase SQL editor or with `supabase db push`.\r\n\r\ncreate extension if not exists pgcrypto","create table if not exists public.conversations (\r\n  id uuid primary key default gen_random_uuid(),\r\n  user_id uuid not null default auth.uid() references auth.users(id) on delete cascade,\r\n  title text not null default 'Obrolan baru',\r\n  selected_model text not null default 'auto'\r\n    check (selected_model in ('auto', 'fast', 'smart', 'document')),\r\n  document_metadata jsonb,\r\n  created_at timestamptz not null default now(),\r\n  updated_at timestamptz not null default now()\r\n)","create table if not exists public.messages (\r\n  id uuid primary key default gen_random_uuid(),\r\n  conversation_id uuid not null references public.conversations(id) on delete cascade,\r\n  role text not null check (role in ('user', 'assistant')),\r\n  content text not null,\r\n  selected_model text not null default 'auto'\r\n    check (selected_model in ('auto', 'fast', 'smart', 'document')),\r\n  document_metadata jsonb,\r\n  created_at timestamptz not null default now()\r\n)","create index if not exists conversations_user_updated_idx\r\n  on public.conversations (user_id, updated_at desc)","create index if not exists messages_conversation_created_idx\r\n  on public.messages (conversation_id, created_at asc)","create or replace function public.set_updated_at()\r\nreturns trigger\r\nlanguage plpgsql\r\nas $$\r\nbegin\r\n  new.updated_at = now();\r\n  return new;\r\nend;\r\n$$","drop trigger if exists conversations_set_updated_at on public.conversations","create trigger conversations_set_updated_at\r\nbefore update on public.conversations\r\nfor each row execute function public.set_updated_at()","create or replace function public.touch_conversation_after_message()\r\nreturns trigger\r\nlanguage plpgsql\r\nsecurity definer\r\nset search_path = public\r\nas $$\r\nbegin\r\n  update public.conversations\r\n  set updated_at = now()\r\n  where id = new.conversation_id;\r\n\r\n  return new;\r\nend;\r\n$$","drop trigger if exists messages_touch_conversation on public.messages","create trigger messages_touch_conversation\r\nafter insert on public.messages\r\nfor each row execute function public.touch_conversation_after_message()","alter table public.conversations enable row level security","alter table public.messages enable row level security","drop policy if exists \\"Users can read own conversations\\" on public.conversations","create policy \\"Users can read own conversations\\"\r\non public.conversations\r\nfor select\r\nusing (auth.uid() = user_id)","drop policy if exists \\"Users can create own conversations\\" on public.conversations","create policy \\"Users can create own conversations\\"\r\non public.conversations\r\nfor insert\r\nwith check (auth.uid() = user_id)","drop policy if exists \\"Users can update own conversations\\" on public.conversations","create policy \\"Users can update own conversations\\"\r\non public.conversations\r\nfor update\r\nusing (auth.uid() = user_id)\r\nwith check (auth.uid() = user_id)","drop policy if exists \\"Users can delete own conversations\\" on public.conversations","create policy \\"Users can delete own conversations\\"\r\non public.conversations\r\nfor delete\r\nusing (auth.uid() = user_id)","drop policy if exists \\"Users can read messages in own conversations\\" on public.messages","create policy \\"Users can read messages in own conversations\\"\r\non public.messages\r\nfor select\r\nusing (\r\n  exists (\r\n    select 1\r\n    from public.conversations\r\n    where conversations.id = messages.conversation_id\r\n      and conversations.user_id = auth.uid()\r\n  )\r\n)","drop policy if exists \\"Users can create messages in own conversations\\" on public.messages","create policy \\"Users can create messages in own conversations\\"\r\non public.messages\r\nfor insert\r\nwith check (\r\n  exists (\r\n    select 1\r\n    from public.conversations\r\n    where conversations.id = messages.conversation_id\r\n      and conversations.user_id = auth.uid()\r\n  )\r\n)","drop policy if exists \\"Users can update messages in own conversations\\" on public.messages","create policy \\"Users can update messages in own conversations\\"\r\non public.messages\r\nfor update\r\nusing (\r\n  exists (\r\n    select 1\r\n    from public.conversations\r\n    where conversations.id = messages.conversation_id\r\n      and conversations.user_id = auth.uid()\r\n  )\r\n)\r\nwith check (\r\n  exists (\r\n    select 1\r\n    from public.conversations\r\n    where conversations.id = messages.conversation_id\r\n      and conversations.user_id = auth.uid()\r\n  )\r\n)","drop policy if exists \\"Users can delete messages in own conversations\\" on public.messages","create policy \\"Users can delete messages in own conversations\\"\r\non public.messages\r\nfor delete\r\nusing (\r\n  exists (\r\n    select 1\r\n    from public.conversations\r\n    where conversations.id = messages.conversation_id\r\n      and conversations.user_id = auth.uid()\r\n  )\r\n)"}	chat_history
20260530010000	{"-- AI Muhammadiyah usage tracking and subscription foundation.\r\n-- Run this after the chat history migration.\r\n\r\ncreate extension if not exists pgcrypto","do $$\r\nbegin\r\n  create type public.subscription_tier as enum (\r\n    'free',\r\n    'kader_pintar',\r\n    'muallim_pro',\r\n    'dakwah_digital',\r\n    'sinergi_ranting'\r\n  );\r\nexception\r\n  when duplicate_object then null;\r\nend $$","do $$\r\nbegin\r\n  create type public.usage_action as enum (\r\n    'message',\r\n    'document_upload'\r\n  );\r\nexception\r\n  when duplicate_object then null;\r\nend $$","create table if not exists public.user_profiles (\r\n  user_id uuid primary key references auth.users(id) on delete cascade,\r\n  display_name text,\r\n  daily_message_count integer not null default 0 check (daily_message_count >= 0),\r\n  daily_upload_count integer not null default 0 check (daily_upload_count >= 0),\r\n  daily_reset_date date not null default current_date,\r\n  created_at timestamptz not null default now(),\r\n  updated_at timestamptz not null default now()\r\n)","create table if not exists public.subscriptions (\r\n  id uuid primary key default gen_random_uuid(),\r\n  user_id uuid not null references auth.users(id) on delete cascade,\r\n  tier public.subscription_tier not null default 'free',\r\n  status text not null default 'active'\r\n    check (status in ('active', 'trialing', 'past_due', 'canceled', 'expired')),\r\n  started_at timestamptz not null default now(),\r\n  current_period_start timestamptz not null default now(),\r\n  current_period_end timestamptz,\r\n  created_at timestamptz not null default now(),\r\n  updated_at timestamptz not null default now()\r\n)","create table if not exists public.usage_logs (\r\n  id uuid primary key default gen_random_uuid(),\r\n  user_id uuid not null references auth.users(id) on delete cascade,\r\n  action public.usage_action not null,\r\n  model_used text not null default 'auto'\r\n    check (model_used in ('auto', 'fast', 'smart', 'document')),\r\n  message_count integer not null default 0 check (message_count >= 0),\r\n  document_count integer not null default 0 check (document_count >= 0),\r\n  estimated_tokens integer not null default 0 check (estimated_tokens >= 0),\r\n  metadata jsonb not null default '{}'::jsonb,\r\n  created_at timestamptz not null default now()\r\n)","create index if not exists subscriptions_user_status_idx\r\n  on public.subscriptions (user_id, status, current_period_end desc nulls first)","create index if not exists usage_logs_user_created_idx\r\n  on public.usage_logs (user_id, created_at desc)","create index if not exists usage_logs_user_action_created_idx\r\n  on public.usage_logs (user_id, action, created_at desc)","drop trigger if exists user_profiles_set_updated_at on public.user_profiles","create trigger user_profiles_set_updated_at\r\nbefore update on public.user_profiles\r\nfor each row execute function public.set_updated_at()","drop trigger if exists subscriptions_set_updated_at on public.subscriptions","create trigger subscriptions_set_updated_at\r\nbefore update on public.subscriptions\r\nfor each row execute function public.set_updated_at()","create or replace function public.ensure_user_profile(p_user_id uuid default auth.uid())\r\nreturns void\r\nlanguage plpgsql\r\nsecurity definer\r\nset search_path = public\r\nas $$\r\nbegin\r\n  if p_user_id is null then\r\n    raise exception 'User is not authenticated.';\r\n  end if;\r\n\r\n  insert into public.user_profiles (user_id)\r\n  values (p_user_id)\r\n  on conflict (user_id) do nothing;\r\n\r\n  insert into public.subscriptions (user_id, tier, status)\r\n  select p_user_id, 'free', 'active'\r\n  where not exists (\r\n    select 1\r\n    from public.subscriptions\r\n    where user_id = p_user_id\r\n      and status in ('active', 'trialing')\r\n      and (current_period_end is null or current_period_end > now())\r\n  );\r\nend;\r\n$$","create or replace function public.handle_new_auth_user()\r\nreturns trigger\r\nlanguage plpgsql\r\nsecurity definer\r\nset search_path = public\r\nas $$\r\nbegin\r\n  perform public.ensure_user_profile(new.id);\r\n  return new;\r\nend;\r\n$$","drop trigger if exists auth_users_create_profile on auth.users","create trigger auth_users_create_profile\r\nafter insert on auth.users\r\nfor each row execute function public.handle_new_auth_user()","create or replace function public.reset_daily_counters(p_user_id uuid default auth.uid())\r\nreturns void\r\nlanguage plpgsql\r\nsecurity definer\r\nset search_path = public\r\nas $$\r\nbegin\r\n  if p_user_id is null then\r\n    raise exception 'User is not authenticated.';\r\n  end if;\r\n\r\n  perform public.ensure_user_profile(p_user_id);\r\n\r\n  update public.user_profiles\r\n  set daily_message_count = 0,\r\n      daily_upload_count = 0,\r\n      daily_reset_date = current_date\r\n  where user_id = p_user_id\r\n    and daily_reset_date < current_date;\r\nend;\r\n$$","create or replace function public.get_current_subscription_tier(\r\n  p_user_id uuid default auth.uid()\r\n)\r\nreturns public.subscription_tier\r\nlanguage sql\r\nsecurity definer\r\nset search_path = public\r\nstable\r\nas $$\r\n  select coalesce(\r\n    (\r\n      select s.tier\r\n      from public.subscriptions s\r\n      where s.user_id = p_user_id\r\n        and s.status in ('active', 'trialing')\r\n        and (s.current_period_end is null or s.current_period_end > now())\r\n      order by\r\n        case s.tier\r\n          when 'sinergi_ranting' then 5\r\n          when 'dakwah_digital' then 4\r\n          when 'muallim_pro' then 3\r\n          when 'kader_pintar' then 2\r\n          else 1\r\n        end desc,\r\n        s.created_at desc\r\n      limit 1\r\n    ),\r\n    'free'::public.subscription_tier\r\n  );\r\n$$","create or replace function public.get_subscription_limits(\r\n  p_tier public.subscription_tier\r\n)\r\nreturns table (\r\n  daily_message_limit integer,\r\n  daily_upload_limit integer,\r\n  allowed_models text[]\r\n)\r\nlanguage sql\r\nstable\r\nas $$\r\n  select daily_message_limit, daily_upload_limit, allowed_models\r\n  from (\r\n    values\r\n      ('free'::public.subscription_tier, 20, 3, array['auto', 'fast']),\r\n      ('kader_pintar'::public.subscription_tier, 100, 10, array['auto', 'fast', 'smart']),\r\n      ('muallim_pro'::public.subscription_tier, 300, 30, array['auto', 'fast', 'smart', 'document']),\r\n      ('dakwah_digital'::public.subscription_tier, 600, 60, array['auto', 'fast', 'smart', 'document']),\r\n      ('sinergi_ranting'::public.subscription_tier, 2000, 200, array['auto', 'fast', 'smart', 'document'])\r\n  ) as limits(tier, daily_message_limit, daily_upload_limit, allowed_models)\r\n  where limits.tier = p_tier;\r\n$$","create or replace function public.get_usage_snapshot(p_user_id uuid default auth.uid())\r\nreturns jsonb\r\nlanguage plpgsql\r\nsecurity definer\r\nset search_path = public\r\nas $$\r\ndeclare\r\n  v_tier public.subscription_tier;\r\n  v_daily_message_limit integer;\r\n  v_daily_upload_limit integer;\r\n  v_allowed_models text[];\r\n  v_messages_used integer;\r\n  v_uploads_used integer;\r\nbegin\r\n  if p_user_id is null then\r\n    raise exception 'User is not authenticated.';\r\n  end if;\r\n\r\n  perform public.reset_daily_counters(p_user_id);\r\n\r\n  v_tier := public.get_current_subscription_tier(p_user_id);\r\n\r\n  select daily_message_limit, daily_upload_limit, allowed_models\r\n  into v_daily_message_limit, v_daily_upload_limit, v_allowed_models\r\n  from public.get_subscription_limits(v_tier);\r\n\r\n  select coalesce(sum(message_count), 0)::integer\r\n  into v_messages_used\r\n  from public.usage_logs\r\n  where user_id = p_user_id\r\n    and action = 'message'\r\n    and created_at >= current_date;\r\n\r\n  select coalesce(sum(document_count), 0)::integer\r\n  into v_uploads_used\r\n  from public.usage_logs\r\n  where user_id = p_user_id\r\n    and action = 'document_upload'\r\n    and created_at >= current_date;\r\n\r\n  return jsonb_build_object(\r\n    'tier', v_tier,\r\n    'daily_message_limit', v_daily_message_limit,\r\n    'daily_upload_limit', v_daily_upload_limit,\r\n    'messages_used_today', v_messages_used,\r\n    'uploads_used_today', v_uploads_used,\r\n    'remaining_messages_today', greatest(v_daily_message_limit - v_messages_used, 0),\r\n    'remaining_uploads_today', greatest(v_daily_upload_limit - v_uploads_used, 0),\r\n    'allowed_models', v_allowed_models\r\n  );\r\nend;\r\n$$","create or replace function public.check_usage_limits(\r\n  p_action public.usage_action,\r\n  p_model_used text default 'auto',\r\n  p_estimated_tokens integer default 0,\r\n  p_user_id uuid default auth.uid()\r\n)\r\nreturns jsonb\r\nlanguage plpgsql\r\nsecurity definer\r\nset search_path = public\r\nas $$\r\ndeclare\r\n  v_snapshot jsonb;\r\n  v_allowed_models text[];\r\nbegin\r\n  if p_user_id is null then\r\n    raise exception 'User is not authenticated.';\r\n  end if;\r\n\r\n  v_snapshot := public.get_usage_snapshot(p_user_id);\r\n  v_allowed_models := array(\r\n    select jsonb_array_elements_text(v_snapshot -> 'allowed_models')\r\n  );\r\n\r\n  if p_action = 'message' and not (p_model_used = any(v_allowed_models)) then\r\n    return v_snapshot || jsonb_build_object(\r\n      'allowed', false,\r\n      'reason', 'model_not_allowed'\r\n    );\r\n  end if;\r\n\r\n  if p_action = 'message'\r\n    and (v_snapshot ->> 'remaining_messages_today')::integer <= 0 then\r\n    return v_snapshot || jsonb_build_object(\r\n      'allowed', false,\r\n      'reason', 'daily_message_limit_exceeded'\r\n    );\r\n  end if;\r\n\r\n  if p_action = 'document_upload'\r\n    and (v_snapshot ->> 'remaining_uploads_today')::integer <= 0 then\r\n    return v_snapshot || jsonb_build_object(\r\n      'allowed', false,\r\n      'reason', 'daily_upload_limit_exceeded'\r\n    );\r\n  end if;\r\n\r\n  return v_snapshot || jsonb_build_object(\r\n    'allowed', true,\r\n    'reason', null,\r\n    'estimated_tokens', greatest(p_estimated_tokens, 0)\r\n  );\r\nend;\r\n$$","create or replace function public.increment_usage(\r\n  p_action public.usage_action,\r\n  p_model_used text default 'auto',\r\n  p_document_count integer default 0,\r\n  p_estimated_tokens integer default 0,\r\n  p_metadata jsonb default '{}'::jsonb,\r\n  p_user_id uuid default auth.uid()\r\n)\r\nreturns jsonb\r\nlanguage plpgsql\r\nsecurity definer\r\nset search_path = public\r\nas $$\r\ndeclare\r\n  v_limit_check jsonb;\r\nbegin\r\n  if p_user_id is null then\r\n    raise exception 'User is not authenticated.';\r\n  end if;\r\n\r\n  v_limit_check := public.check_usage_limits(\r\n    p_action,\r\n    p_model_used,\r\n    p_estimated_tokens,\r\n    p_user_id\r\n  );\r\n\r\n  if not (v_limit_check ->> 'allowed')::boolean then\r\n    raise exception 'Usage limit exceeded: %', v_limit_check ->> 'reason';\r\n  end if;\r\n\r\n  insert into public.usage_logs (\r\n    user_id,\r\n    action,\r\n    model_used,\r\n    message_count,\r\n    document_count,\r\n    estimated_tokens,\r\n    metadata\r\n  )\r\n  values (\r\n    p_user_id,\r\n    p_action,\r\n    p_model_used,\r\n    case when p_action = 'message' then 1 else 0 end,\r\n    case when p_action = 'document_upload' then greatest(p_document_count, 1) else 0 end,\r\n    greatest(p_estimated_tokens, 0),\r\n    coalesce(p_metadata, '{}'::jsonb)\r\n  );\r\n\r\n  update public.user_profiles\r\n  set daily_message_count = daily_message_count\r\n      + case when p_action = 'message' then 1 else 0 end,\r\n      daily_upload_count = daily_upload_count\r\n      + case when p_action = 'document_upload' then greatest(p_document_count, 1) else 0 end\r\n  where user_id = p_user_id;\r\n\r\n  return public.get_usage_snapshot(p_user_id);\r\nend;\r\n$$","alter table public.user_profiles enable row level security","alter table public.subscriptions enable row level security","alter table public.usage_logs enable row level security","drop policy if exists \\"Users can read own profile\\" on public.user_profiles","create policy \\"Users can read own profile\\"\r\non public.user_profiles\r\nfor select\r\nusing (auth.uid() = user_id)","drop policy if exists \\"Users can update own profile\\" on public.user_profiles","create policy \\"Users can update own profile\\"\r\non public.user_profiles\r\nfor update\r\nusing (auth.uid() = user_id)\r\nwith check (auth.uid() = user_id)","drop policy if exists \\"Users can read own subscriptions\\" on public.subscriptions","create policy \\"Users can read own subscriptions\\"\r\non public.subscriptions\r\nfor select\r\nusing (auth.uid() = user_id)","drop policy if exists \\"Users can read own usage logs\\" on public.usage_logs","create policy \\"Users can read own usage logs\\"\r\non public.usage_logs\r\nfor select\r\nusing (auth.uid() = user_id)"}	usage_and_subscriptions
20260530020000	{"-- Repair usage defaults so every authenticated user has a usable free tier.\r\n-- Run this after 20260530010000_usage_and_subscriptions.sql.\r\n\r\ncreate extension if not exists pgcrypto","create or replace function public.ensure_user_profile(p_user_id uuid default auth.uid())\r\nreturns void\r\nlanguage plpgsql\r\nsecurity definer\r\nset search_path = public\r\nas $$\r\nbegin\r\n  if p_user_id is null then\r\n    raise exception 'User is not authenticated.';\r\n  end if;\r\n\r\n  insert into public.user_profiles (user_id, daily_message_count, daily_upload_count, daily_reset_date)\r\n  values (p_user_id, 0, 0, current_date)\r\n  on conflict (user_id) do nothing;\r\n\r\n  insert into public.subscriptions (user_id, tier, status)\r\n  select p_user_id, 'free'::public.subscription_tier, 'active'\r\n  where not exists (\r\n    select 1\r\n    from public.subscriptions\r\n    where user_id = p_user_id\r\n      and status in ('active', 'trialing')\r\n      and (current_period_end is null or current_period_end > now())\r\n  );\r\nend;\r\n$$","insert into public.user_profiles (user_id, daily_message_count, daily_upload_count, daily_reset_date)\r\nselect users.id, 0, 0, current_date\r\nfrom auth.users\r\nleft join public.user_profiles profiles on profiles.user_id = users.id\r\nwhere profiles.user_id is null","insert into public.subscriptions (user_id, tier, status)\r\nselect users.id, 'free'::public.subscription_tier, 'active'\r\nfrom auth.users\r\nwhere not exists (\r\n  select 1\r\n  from public.subscriptions subscriptions\r\n  where subscriptions.user_id = users.id\r\n    and subscriptions.status in ('active', 'trialing')\r\n    and (\r\n      subscriptions.current_period_end is null\r\n      or subscriptions.current_period_end > now()\r\n    )\r\n)","create or replace function public.handle_new_auth_user()\r\nreturns trigger\r\nlanguage plpgsql\r\nsecurity definer\r\nset search_path = public\r\nas $$\r\nbegin\r\n  perform public.ensure_user_profile(new.id);\r\n  return new;\r\nend;\r\n$$","drop trigger if exists auth_users_create_profile on auth.users","create trigger auth_users_create_profile\r\nafter insert on auth.users\r\nfor each row execute function public.handle_new_auth_user()","create or replace function public.get_subscription_limits(\r\n  p_tier public.subscription_tier\r\n)\r\nreturns table (\r\n  daily_message_limit integer,\r\n  daily_upload_limit integer,\r\n  allowed_models text[]\r\n)\r\nlanguage sql\r\nstable\r\nas $$\r\n  select daily_message_limit, daily_upload_limit, allowed_models\r\n  from (\r\n    values\r\n      ('free'::public.subscription_tier, 20, 3, array['auto', 'fast']),\r\n      ('kader_pintar'::public.subscription_tier, 100, 10, array['auto', 'fast', 'smart']),\r\n      ('muallim_pro'::public.subscription_tier, 300, 30, array['auto', 'fast', 'smart', 'document']),\r\n      ('dakwah_digital'::public.subscription_tier, 600, 60, array['auto', 'fast', 'smart', 'document']),\r\n      ('sinergi_ranting'::public.subscription_tier, 2000, 200, array['auto', 'fast', 'smart', 'document'])\r\n  ) as limits(tier, daily_message_limit, daily_upload_limit, allowed_models)\r\n  where limits.tier = coalesce(p_tier, 'free'::public.subscription_tier);\r\n$$","create or replace function public.get_usage_snapshot(p_user_id uuid default auth.uid())\r\nreturns jsonb\r\nlanguage plpgsql\r\nsecurity definer\r\nset search_path = public\r\nas $$\r\ndeclare\r\n  v_tier public.subscription_tier;\r\n  v_daily_message_limit integer := 20;\r\n  v_daily_upload_limit integer := 3;\r\n  v_allowed_models text[] := array['auto', 'fast'];\r\n  v_messages_used integer := 0;\r\n  v_uploads_used integer := 0;\r\nbegin\r\n  if p_user_id is null then\r\n    raise exception 'User is not authenticated.';\r\n  end if;\r\n\r\n  perform public.reset_daily_counters(p_user_id);\r\n\r\n  v_tier := coalesce(\r\n    public.get_current_subscription_tier(p_user_id),\r\n    'free'::public.subscription_tier\r\n  );\r\n\r\n  select\r\n    coalesce(limits.daily_message_limit, 20),\r\n    coalesce(limits.daily_upload_limit, 3),\r\n    coalesce(limits.allowed_models, array['auto', 'fast'])\r\n  into v_daily_message_limit, v_daily_upload_limit, v_allowed_models\r\n  from public.get_subscription_limits(v_tier) limits;\r\n\r\n  v_daily_message_limit := greatest(coalesce(v_daily_message_limit, 20), 1);\r\n  v_daily_upload_limit := greatest(coalesce(v_daily_upload_limit, 3), 1);\r\n  v_allowed_models := coalesce(v_allowed_models, array['auto', 'fast']);\r\n\r\n  select coalesce(sum(message_count), 0)::integer\r\n  into v_messages_used\r\n  from public.usage_logs\r\n  where user_id = p_user_id\r\n    and action = 'message'\r\n    and created_at >= current_date;\r\n\r\n  select coalesce(sum(document_count), 0)::integer\r\n  into v_uploads_used\r\n  from public.usage_logs\r\n  where user_id = p_user_id\r\n    and action = 'document_upload'\r\n    and created_at >= current_date;\r\n\r\n  return jsonb_build_object(\r\n    'tier', v_tier,\r\n    'daily_message_limit', v_daily_message_limit,\r\n    'daily_upload_limit', v_daily_upload_limit,\r\n    'messages_used_today', v_messages_used,\r\n    'uploads_used_today', v_uploads_used,\r\n    'remaining_messages_today', greatest(v_daily_message_limit - v_messages_used, 0),\r\n    'remaining_uploads_today', greatest(v_daily_upload_limit - v_uploads_used, 0),\r\n    'allowed_models', v_allowed_models\r\n  );\r\nend;\r\n$$"}	fix_usage_defaults
20260530030000	{"-- AI Muhammadiyah lightweight learning profile memory.\r\n-- Run this after 20260530020000_fix_usage_defaults.sql.\r\n\r\ncreate table if not exists public.user_memory (\r\n  user_id uuid primary key references auth.users(id) on delete cascade,\r\n  display_name text check (char_length(display_name) <= 80),\r\n  school_level text check (char_length(school_level) <= 80),\r\n  learning_goals text check (char_length(learning_goals) <= 400),\r\n  favorite_subjects text[] not null default '{}'::text[]\r\n    check (array_length(favorite_subjects, 1) is null or array_length(favorite_subjects, 1) <= 8),\r\n  preferred_language text check (char_length(preferred_language) <= 60),\r\n  preferred_explanation_style text check (char_length(preferred_explanation_style) <= 220),\r\n  created_at timestamptz not null default now(),\r\n  updated_at timestamptz not null default now()\r\n)","create index if not exists user_memory_user_id_idx\r\n  on public.user_memory (user_id)","drop trigger if exists user_memory_set_updated_at on public.user_memory","create trigger user_memory_set_updated_at\r\nbefore update on public.user_memory\r\nfor each row execute function public.set_updated_at()","alter table public.user_memory enable row level security","drop policy if exists \\"Users can read own memory\\" on public.user_memory","create policy \\"Users can read own memory\\"\r\non public.user_memory\r\nfor select\r\nusing (auth.uid() = user_id)","drop policy if exists \\"Users can create own memory\\" on public.user_memory","create policy \\"Users can create own memory\\"\r\non public.user_memory\r\nfor insert\r\nwith check (auth.uid() = user_id)","drop policy if exists \\"Users can update own memory\\" on public.user_memory","create policy \\"Users can update own memory\\"\r\non public.user_memory\r\nfor update\r\nusing (auth.uid() = user_id)\r\nwith check (auth.uid() = user_id)"}	user_memory
20260531000000	{"-- AI Muhammadiyah settings preferences stored with the existing learning memory.\r\n\r\nalter table public.user_memory\r\n  add column if not exists theme_preference text not null default 'system'\r\n    check (theme_preference in ('system', 'light', 'dark')),\r\n  add column if not exists default_model text not null default 'auto'\r\n    check (default_model in ('auto', 'fast', 'smart', 'document')),\r\n  add column if not exists default_study_mode text not null default 'Kajian umum'\r\n    check (char_length(default_study_mode) <= 80)"}	user_settings_preferences
20260601000000	{"-- Persist AI Muhammadiyah study mode choices on chat history.\r\n\r\nalter table public.conversations\r\n  add column if not exists study_mode text not null default 'cambridge_tutor'\r\n    check (\r\n      study_mode in (\r\n        'quick_explain',\r\n        'cambridge_tutor',\r\n        'osn_coach',\r\n        'islamic_teacher',\r\n        'coding_mentor',\r\n        'research_mode',\r\n        'step_by_step'\r\n      )\r\n    )","alter table public.messages\r\n  add column if not exists study_mode text not null default 'cambridge_tutor'\r\n    check (\r\n      study_mode in (\r\n        'quick_explain',\r\n        'cambridge_tutor',\r\n        'osn_coach',\r\n        'islamic_teacher',\r\n        'coding_mentor',\r\n        'research_mode',\r\n        'step_by_step'\r\n      )\r\n    )","alter table public.user_memory\r\n  alter column default_study_mode set default 'cambridge_tutor'","update public.user_memory\r\nset default_study_mode = case default_study_mode\r\n  when 'Tanya jawab cepat' then 'quick_explain'\r\n  when 'Latihan soal' then 'step_by_step'\r\n  when 'Kajian umum' then 'cambridge_tutor'\r\n  when 'Ringkasan materi' then 'cambridge_tutor'\r\n  when 'Persiapan ujian' then 'cambridge_tutor'\r\n  else default_study_mode\r\nend\r\nwhere default_study_mode in (\r\n  'Tanya jawab cepat',\r\n  'Latihan soal',\r\n  'Kajian umum',\r\n  'Ringkasan materi',\r\n  'Persiapan ujian'\r\n)"}	study_modes
20260601010000	{"-- Phase 1 RAG knowledge base for AI Muhammadiyah.\r\n-- Uses PostgreSQL full-text search first; pgvector can be added later without\r\n-- changing the source/chunk ownership model.\r\n\r\ncreate table if not exists public.knowledge_sources (\r\n  id uuid primary key default gen_random_uuid(),\r\n  title text not null check (char_length(title) <= 240),\r\n  category text not null default 'general' check (char_length(category) <= 120),\r\n  file_type text not null check (file_type in ('pdf', 'docx', 'pptx', 'xlsx')),\r\n  original_file_name text,\r\n  status text not null default 'active'\r\n    check (status in ('active', 'draft', 'archived')),\r\n  is_public boolean not null default true,\r\n  chunk_count integer not null default 0 check (chunk_count >= 0),\r\n  metadata jsonb not null default '{}'::jsonb,\r\n  created_by uuid references auth.users(id) on delete set null,\r\n  created_at timestamptz not null default now(),\r\n  updated_at timestamptz not null default now()\r\n)","create table if not exists public.knowledge_chunks (\r\n  id uuid primary key default gen_random_uuid(),\r\n  source_id uuid not null references public.knowledge_sources(id) on delete cascade,\r\n  chunk_order integer not null check (chunk_order >= 0),\r\n  content text not null,\r\n  search_vector tsvector generated always as (to_tsvector('simple', content)) stored,\r\n  created_by uuid references auth.users(id) on delete set null,\r\n  created_at timestamptz not null default now(),\r\n  unique (source_id, chunk_order)\r\n)","create index if not exists knowledge_sources_status_public_idx\r\n  on public.knowledge_sources (status, is_public, created_at desc)","create index if not exists knowledge_chunks_source_order_idx\r\n  on public.knowledge_chunks (source_id, chunk_order)","create index if not exists knowledge_chunks_search_idx\r\n  on public.knowledge_chunks using gin (search_vector)","drop trigger if exists knowledge_sources_set_updated_at on public.knowledge_sources","create trigger knowledge_sources_set_updated_at\r\nbefore update on public.knowledge_sources\r\nfor each row\r\nexecute function public.set_updated_at()","create or replace function public.search_knowledge_chunks(\r\n  p_query text,\r\n  p_limit integer default 4\r\n)\r\nreturns table (\r\n  source_id uuid,\r\n  source_title text,\r\n  category text,\r\n  chunk_order integer,\r\n  content text,\r\n  rank real\r\n)\r\nlanguage sql\r\nstable\r\nsecurity invoker\r\nset search_path = public\r\nas $$\r\n  with query as (\r\n    select plainto_tsquery('simple', coalesce(nullif(trim(p_query), ''), ' ')) as value\r\n  )\r\n  select\r\n    sources.id as source_id,\r\n    sources.title as source_title,\r\n    sources.category,\r\n    chunks.chunk_order,\r\n    chunks.content,\r\n    ts_rank(chunks.search_vector, query.value) as rank\r\n  from public.knowledge_chunks chunks\r\n  join public.knowledge_sources sources on sources.id = chunks.source_id\r\n  cross join query\r\n  where sources.status = 'active'\r\n    and sources.is_public = true\r\n    and chunks.search_vector @@ query.value\r\n  order by rank desc, sources.created_at desc, chunks.chunk_order asc\r\n  limit greatest(1, least(coalesce(p_limit, 4), 8));\r\n$$","alter table public.knowledge_sources enable row level security","alter table public.knowledge_chunks enable row level security","drop policy if exists \\"Authenticated users can read active public knowledge sources\\"\r\n  on public.knowledge_sources","create policy \\"Authenticated users can read active public knowledge sources\\"\r\non public.knowledge_sources\r\nfor select\r\nto authenticated\r\nusing (status = 'active' and is_public = true)","drop policy if exists \\"Authenticated users can read active public knowledge chunks\\"\r\n  on public.knowledge_chunks","create policy \\"Authenticated users can read active public knowledge chunks\\"\r\non public.knowledge_chunks\r\nfor select\r\nto authenticated\r\nusing (\r\n  exists (\r\n    select 1\r\n    from public.knowledge_sources sources\r\n    where sources.id = knowledge_chunks.source_id\r\n      and sources.status = 'active'\r\n      and sources.is_public = true\r\n  )\r\n)","drop policy if exists \\"Knowledge admins can manage knowledge sources\\"\r\n  on public.knowledge_sources","create policy \\"Knowledge admins can manage knowledge sources\\"\r\non public.knowledge_sources\r\nfor all\r\nto authenticated\r\nusing (\r\n  coalesce((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin', false)\r\n  or coalesce((auth.jwt() -> 'app_metadata' ->> 'knowledge_admin')::boolean, false)\r\n)\r\nwith check (\r\n  coalesce((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin', false)\r\n  or coalesce((auth.jwt() -> 'app_metadata' ->> 'knowledge_admin')::boolean, false)\r\n)","drop policy if exists \\"Knowledge admins can manage knowledge chunks\\"\r\n  on public.knowledge_chunks","create policy \\"Knowledge admins can manage knowledge chunks\\"\r\non public.knowledge_chunks\r\nfor all\r\nto authenticated\r\nusing (\r\n  coalesce((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin', false)\r\n  or coalesce((auth.jwt() -> 'app_metadata' ->> 'knowledge_admin')::boolean, false)\r\n)\r\nwith check (\r\n  coalesce((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin', false)\r\n  or coalesce((auth.jwt() -> 'app_metadata' ->> 'knowledge_admin')::boolean, false)\r\n)"}	knowledge_base
20260602000000	{"-- Workspace and organization metadata for AI Muhammadiyah chats.\r\n-- Safe for existing conversations: old rows remain in the General workspace.\r\n\r\ncreate extension if not exists pgcrypto","create table if not exists public.chat_workspaces (\r\n  id uuid primary key default gen_random_uuid(),\r\n  user_id uuid not null default auth.uid() references auth.users(id) on delete cascade,\r\n  name text not null,\r\n  created_at timestamptz not null default now(),\r\n  updated_at timestamptz not null default now(),\r\n  constraint chat_workspaces_name_not_blank check (length(btrim(name)) > 0),\r\n  constraint chat_workspaces_user_name_unique unique (user_id, name)\r\n)","alter table public.conversations\r\n  add column if not exists workspace_id uuid references public.chat_workspaces(id) on delete set null,\r\n  add column if not exists is_pinned boolean not null default false","create index if not exists chat_workspaces_user_name_idx\r\n  on public.chat_workspaces (user_id, name)","create index if not exists conversations_user_pinned_updated_idx\r\n  on public.conversations (user_id, is_pinned desc, updated_at desc)","create index if not exists conversations_workspace_updated_idx\r\n  on public.conversations (workspace_id, updated_at desc)","create or replace function public.ensure_conversation_workspace_owner()\r\nreturns trigger\r\nlanguage plpgsql\r\nsecurity definer\r\nset search_path = public\r\nas $$\r\nbegin\r\n  if new.workspace_id is null then\r\n    return new;\r\n  end if;\r\n\r\n  if not exists (\r\n    select 1\r\n    from public.chat_workspaces\r\n    where chat_workspaces.id = new.workspace_id\r\n      and chat_workspaces.user_id = new.user_id\r\n  ) then\r\n    raise exception 'Conversation workspace must belong to the same user.';\r\n  end if;\r\n\r\n  return new;\r\nend;\r\n$$","drop trigger if exists conversations_workspace_owner on public.conversations","create trigger conversations_workspace_owner\r\nbefore insert or update of workspace_id, user_id on public.conversations\r\nfor each row execute function public.ensure_conversation_workspace_owner()","drop trigger if exists chat_workspaces_set_updated_at on public.chat_workspaces","create trigger chat_workspaces_set_updated_at\r\nbefore update on public.chat_workspaces\r\nfor each row execute function public.set_updated_at()","alter table public.chat_workspaces enable row level security","drop policy if exists \\"Users can read own chat workspaces\\" on public.chat_workspaces","create policy \\"Users can read own chat workspaces\\"\r\non public.chat_workspaces\r\nfor select\r\nusing (auth.uid() = user_id)","drop policy if exists \\"Users can create own chat workspaces\\" on public.chat_workspaces","create policy \\"Users can create own chat workspaces\\"\r\non public.chat_workspaces\r\nfor insert\r\nwith check (auth.uid() = user_id)","drop policy if exists \\"Users can update own chat workspaces\\" on public.chat_workspaces","create policy \\"Users can update own chat workspaces\\"\r\non public.chat_workspaces\r\nfor update\r\nusing (auth.uid() = user_id)\r\nwith check (auth.uid() = user_id)","drop policy if exists \\"Users can delete own chat workspaces\\" on public.chat_workspaces","create policy \\"Users can delete own chat workspaces\\"\r\non public.chat_workspaces\r\nfor delete\r\nusing (auth.uid() = user_id)"}	chat_workspaces
20260602010000	{"-- Incremental production performance indexes for AI Muhammadiyah.\r\n-- Safe to run after the workspace migration: no tables or columns are recreated.\r\n\r\ncreate extension if not exists pg_trgm","-- Workspace and recency filters used by the chat sidebar and workspace views.\r\ncreate index if not exists conversations_workspace_idx\r\n  on public.conversations (workspace_id)","create index if not exists conversations_updated_at_idx\r\n  on public.conversations (updated_at desc)","-- Chat search uses ilike on conversation titles and message content.\r\ncreate index if not exists conversations_title_trgm_idx\r\n  on public.conversations using gin (title gin_trgm_ops)","create index if not exists messages_content_trgm_idx\r\n  on public.messages using gin (content gin_trgm_ops)","-- Explicit conversation lookup index for message reads and joins.\r\n-- The earlier chat history migration also adds conversation_id + created_at;\r\n-- this narrower index is kept for direct equality filters.\r\ncreate index if not exists messages_conversation_id_idx\r\n  on public.messages (conversation_id)"}	performance_indexes
20260704000000	{"-- AI Muhammadiyah: Skills + Docs/Tasks/Sheets/Canvas (4 tools baru) + skill_change_log.\n-- Extends chat_workspaces (icon/color/skill_id/persona_config) instead of renaming it.\n-- hub_links is intentionally NOT created here (deferred to a later phase).\n-- Review before applying: this file is not run automatically.\n\ncreate extension if not exists pgcrypto","-- ---------------------------------------------------------------------------\n-- 1. Extend chat_workspaces\n-- ---------------------------------------------------------------------------\n\nalter table public.chat_workspaces\n  add column if not exists icon text,\n  add column if not exists color text,\n  add column if not exists persona_config jsonb not null default '{}'::jsonb","-- ---------------------------------------------------------------------------\n-- 2. skills\n-- ---------------------------------------------------------------------------\n\ncreate table if not exists public.skills (\n  id uuid primary key default gen_random_uuid(),\n  owner_id uuid references auth.users(id) on delete cascade,\n  name text not null check (length(btrim(name)) > 0),\n  category text check (char_length(category) <= 120),\n  system_prompt text not null,\n  is_custom boolean not null default false,\n  created_at timestamptz not null default now(),\n  updated_at timestamptz not null default now(),\n  constraint skills_owner_is_custom_match check (\n    (owner_id is null and is_custom = false)\n    or (owner_id is not null and is_custom = true)\n  )\n)","create index if not exists skills_owner_id_idx\n  on public.skills (owner_id)","drop trigger if exists skills_set_updated_at on public.skills","create trigger skills_set_updated_at\nbefore update on public.skills\nfor each row execute function public.set_updated_at()","alter table public.skills enable row level security","drop policy if exists \\"Authenticated users can read platform and own skills\\" on public.skills","create policy \\"Authenticated users can read platform and own skills\\"\non public.skills\nfor select\nto authenticated\nusing (owner_id is null or owner_id = auth.uid())","drop policy if exists \\"Users can create own custom skills\\" on public.skills","create policy \\"Users can create own custom skills\\"\non public.skills\nfor insert\nto authenticated\nwith check (owner_id = auth.uid() and is_custom = true)","drop policy if exists \\"Users can update own custom skills\\" on public.skills","create policy \\"Users can update own custom skills\\"\non public.skills\nfor update\nto authenticated\nusing (owner_id = auth.uid())\nwith check (owner_id = auth.uid() and is_custom = true)","drop policy if exists \\"Users can delete own custom skills\\" on public.skills","create policy \\"Users can delete own custom skills\\"\non public.skills\nfor delete\nto authenticated\nusing (owner_id = auth.uid())","drop policy if exists \\"Skill admins can manage platform skills\\" on public.skills","create policy \\"Skill admins can manage platform skills\\"\non public.skills\nfor all\nto authenticated\nusing (\n  coalesce((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin', false)\n  or coalesce((auth.jwt() -> 'app_metadata' ->> 'skill_admin')::boolean, false)\n)\nwith check (\n  coalesce((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin', false)\n  or coalesce((auth.jwt() -> 'app_metadata' ->> 'skill_admin')::boolean, false)\n)","-- ---------------------------------------------------------------------------\n-- 3. chat_workspaces.skill_id (added after skills exists so the FK can resolve)\n-- ---------------------------------------------------------------------------\n\nalter table public.chat_workspaces\n  add column if not exists skill_id uuid references public.skills(id) on delete set null","create index if not exists chat_workspaces_skill_id_idx\n  on public.chat_workspaces (skill_id)","-- ---------------------------------------------------------------------------\n-- 4. docs / tasks / sheets / canvases\n--    Ownership is inherited from chat_workspaces (workspace_id -> chat_workspaces.user_id),\n--    mirroring the existing messages -> conversations RLS pattern.\n-- ---------------------------------------------------------------------------\n\ncreate table if not exists public.docs (\n  id uuid primary key default gen_random_uuid(),\n  workspace_id uuid not null references public.chat_workspaces(id) on delete cascade,\n  title text not null default 'Untitled',\n  content text not null default '',\n  source_ref uuid references public.conversations(id) on delete set null,\n  created_at timestamptz not null default now(),\n  updated_at timestamptz not null default now()\n)","create table if not exists public.tasks (\n  id uuid primary key default gen_random_uuid(),\n  workspace_id uuid not null references public.chat_workspaces(id) on delete cascade,\n  title text not null default 'Untitled',\n  tasks jsonb not null default '[]'::jsonb,\n  source_ref uuid references public.conversations(id) on delete set null,\n  created_at timestamptz not null default now(),\n  updated_at timestamptz not null default now()\n)","create table if not exists public.sheets (\n  id uuid primary key default gen_random_uuid(),\n  workspace_id uuid not null references public.chat_workspaces(id) on delete cascade,\n  title text not null default 'Untitled',\n  data jsonb not null default '[]'::jsonb,\n  source_ref uuid references public.conversations(id) on delete set null,\n  created_at timestamptz not null default now(),\n  updated_at timestamptz not null default now()\n)","create table if not exists public.canvases (\n  id uuid primary key default gen_random_uuid(),\n  workspace_id uuid not null references public.chat_workspaces(id) on delete cascade,\n  title text not null default 'Untitled',\n  nodes jsonb not null default '[]'::jsonb,\n  source_ref uuid references public.conversations(id) on delete set null,\n  created_at timestamptz not null default now(),\n  updated_at timestamptz not null default now()\n)","create index if not exists docs_workspace_updated_idx\n  on public.docs (workspace_id, updated_at desc)","create index if not exists tasks_workspace_updated_idx\n  on public.tasks (workspace_id, updated_at desc)","create index if not exists sheets_workspace_updated_idx\n  on public.sheets (workspace_id, updated_at desc)","create index if not exists canvases_workspace_updated_idx\n  on public.canvases (workspace_id, updated_at desc)","drop trigger if exists docs_set_updated_at on public.docs","create trigger docs_set_updated_at\nbefore update on public.docs\nfor each row execute function public.set_updated_at()","drop trigger if exists tasks_set_updated_at on public.tasks","create trigger tasks_set_updated_at\nbefore update on public.tasks\nfor each row execute function public.set_updated_at()","drop trigger if exists sheets_set_updated_at on public.sheets","create trigger sheets_set_updated_at\nbefore update on public.sheets\nfor each row execute function public.set_updated_at()","drop trigger if exists canvases_set_updated_at on public.canvases","create trigger canvases_set_updated_at\nbefore update on public.canvases\nfor each row execute function public.set_updated_at()","alter table public.docs enable row level security","alter table public.tasks enable row level security","alter table public.sheets enable row level security","alter table public.canvases enable row level security","-- docs policies\ndrop policy if exists \\"Users can read own docs\\" on public.docs","create policy \\"Users can read own docs\\"\non public.docs\nfor select\nusing (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = docs.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","drop policy if exists \\"Users can create own docs\\" on public.docs","create policy \\"Users can create own docs\\"\non public.docs\nfor insert\nwith check (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = docs.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","drop policy if exists \\"Users can update own docs\\" on public.docs","create policy \\"Users can update own docs\\"\non public.docs\nfor update\nusing (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = docs.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)\nwith check (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = docs.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","drop policy if exists \\"Users can delete own docs\\" on public.docs","create policy \\"Users can delete own docs\\"\non public.docs\nfor delete\nusing (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = docs.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","-- tasks policies\ndrop policy if exists \\"Users can read own tasks\\" on public.tasks","create policy \\"Users can read own tasks\\"\non public.tasks\nfor select\nusing (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = tasks.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","drop policy if exists \\"Users can create own tasks\\" on public.tasks","create policy \\"Users can create own tasks\\"\non public.tasks\nfor insert\nwith check (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = tasks.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","drop policy if exists \\"Users can update own tasks\\" on public.tasks","create policy \\"Users can update own tasks\\"\non public.tasks\nfor update\nusing (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = tasks.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)\nwith check (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = tasks.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","drop policy if exists \\"Users can delete own tasks\\" on public.tasks","create policy \\"Users can delete own tasks\\"\non public.tasks\nfor delete\nusing (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = tasks.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","-- sheets policies\ndrop policy if exists \\"Users can read own sheets\\" on public.sheets","create policy \\"Users can read own sheets\\"\non public.sheets\nfor select\nusing (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = sheets.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","drop policy if exists \\"Users can create own sheets\\" on public.sheets","create policy \\"Users can create own sheets\\"\non public.sheets\nfor insert\nwith check (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = sheets.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","drop policy if exists \\"Users can update own sheets\\" on public.sheets","create policy \\"Users can update own sheets\\"\non public.sheets\nfor update\nusing (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = sheets.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)\nwith check (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = sheets.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","drop policy if exists \\"Users can delete own sheets\\" on public.sheets","create policy \\"Users can delete own sheets\\"\non public.sheets\nfor delete\nusing (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = sheets.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","-- canvases policies\ndrop policy if exists \\"Users can read own canvases\\" on public.canvases","create policy \\"Users can read own canvases\\"\non public.canvases\nfor select\nusing (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = canvases.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","drop policy if exists \\"Users can create own canvases\\" on public.canvases","create policy \\"Users can create own canvases\\"\non public.canvases\nfor insert\nwith check (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = canvases.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","drop policy if exists \\"Users can update own canvases\\" on public.canvases","create policy \\"Users can update own canvases\\"\non public.canvases\nfor update\nusing (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = canvases.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)\nwith check (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = canvases.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","drop policy if exists \\"Users can delete own canvases\\" on public.canvases","create policy \\"Users can delete own canvases\\"\non public.canvases\nfor delete\nusing (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = canvases.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","-- ---------------------------------------------------------------------------\n-- 5. skill_change_log (append-only audit trail; no update/delete policies)\n-- ---------------------------------------------------------------------------\n\ncreate table if not exists public.skill_change_log (\n  id uuid primary key default gen_random_uuid(),\n  workspace_id uuid not null references public.chat_workspaces(id) on delete cascade,\n  skill_id uuid references public.skills(id) on delete set null,\n  conversation_id uuid references public.conversations(id) on delete set null,\n  changed_at timestamptz not null default now()\n)","create index if not exists skill_change_log_workspace_changed_idx\n  on public.skill_change_log (workspace_id, changed_at desc)","alter table public.skill_change_log enable row level security","drop policy if exists \\"Users can read own skill change log\\" on public.skill_change_log","create policy \\"Users can read own skill change log\\"\non public.skill_change_log\nfor select\nusing (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = skill_change_log.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)","drop policy if exists \\"Users can append own skill change log\\" on public.skill_change_log","create policy \\"Users can append own skill change log\\"\non public.skill_change_log\nfor insert\nwith check (\n  exists (\n    select 1\n    from public.chat_workspaces\n    where chat_workspaces.id = skill_change_log.workspace_id\n      and chat_workspaces.user_id = auth.uid()\n  )\n)"}	docs_tasks_sheets_canvases
\.
...
--
-- Name: skill_change_log skill_change_log_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.skill_change_log
    ADD CONSTRAINT skill_change_log_pkey PRIMARY KEY (id);
...
--
-- Name: canvases_workspace_updated_idx; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX canvases_workspace_updated_idx ON public.canvases USING btree (workspace_id, updated_at DESC);


--
-- Name: chat_workspaces_skill_id_idx; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX chat_workspaces_skill_id_idx ON public.chat_workspaces USING btree (skill_id);
...
--
-- Name: docs_workspace_updated_idx; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX docs_workspace_updated_idx ON public.docs USING btree (workspace_id, updated_at DESC);
...
--
-- Name: sheets_workspace_updated_idx; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX sheets_workspace_updated_idx ON public.sheets USING btree (workspace_id, updated_at DESC);


--
-- Name: skill_change_log_workspace_changed_idx; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX skill_change_log_workspace_changed_idx ON public.skill_change_log USING btree (workspace_id, changed_at DESC);


--
-- Name: skills_owner_id_idx; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX skills_owner_id_idx ON public.skills USING btree (owner_id);
...
--
-- Name: tasks_workspace_updated_idx; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX tasks_workspace_updated_idx ON public.tasks USING btree (workspace_id, updated_at DESC);
...
--
-- Name: canvases canvases_set_updated_at; Type: TRIGGER; Schema: public; Owner: postgres
--

CREATE TRIGGER canvases_set_updated_at BEFORE UPDATE ON public.canvases FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
--
-- Name: docs docs_set_updated_at; Type: TRIGGER; Schema: public; Owner: postgres
--

CREATE TRIGGER docs_set_updated_at BEFORE UPDATE ON public.docs FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
--
-- Name: sheets sheets_set_updated_at; Type: TRIGGER; Schema: public; Owner: postgres
--
...
--
-- Name: tasks tasks_set_updated_at; Type: TRIGGER; Schema: public; Owner: postgres
--
...
--
-- Name: canvases canvases_source_ref_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.canvases
    ADD CONSTRAINT canvases_source_ref_fkey FOREIGN KEY (source_ref) REFERENCES public.conversations(id) ON DELETE SET NULL;


--
-- Name: canvases canvases_workspace_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.canvases
    ADD CONSTRAINT canvases_workspace_id_fkey FOREIGN KEY (workspace_id) REFERENCES public.chat_workspaces(id) ON DELETE CASCADE;


--
-- Name: chat_workspaces chat_workspaces_skill_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chat_workspaces
    ADD CONSTRAINT chat_workspaces_skill_id_fkey FOREIGN KEY (skill_id) REFERENCES public.skills(id) ON DELETE SET NULL;
...
--
-- Name: docs docs_source_ref_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.docs
    ADD CONSTRAINT docs_source_ref_fkey FOREIGN KEY (source_ref) REFERENCES public.conversations(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY public.docs
    ADD CONSTRAINT docs_workspace_id_fkey FOREIGN KEY (workspace_id) REFERENCES public.chat_workspaces(id) ON DELETE CASCADE;
...
--
-- Name: sheets sheets_source_ref_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.sheets
    ADD CONSTRAINT sheets_source_ref_fkey FOREIGN KEY (source_ref) REFERENCES public.conversations(id) ON DELETE SET NULL;


--
-- Name: sheets sheets_workspace_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.sheets
    ADD CONSTRAINT sheets_workspace_id_fkey FOREIGN KEY (workspace_id) REFERENCES public.chat_workspaces(id) ON DELETE CASCADE;


--
-- Name: skill_change_log skill_change_log_conversation_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.skill_change_log
    ADD CONSTRAINT skill_change_log_conversation_id_fkey FOREIGN KEY (conversation_id) REFERENCES public.conversations(id) ON DELETE SET NULL;


--
-- Name: skill_change_log skill_change_log_skill_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.skill_change_log
    ADD CONSTRAINT skill_change_log_skill_id_fkey FOREIGN KEY (skill_id) REFERENCES public.skills(id) ON DELETE SET NULL;


--
-- Name: skill_change_log skill_change_log_workspace_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.skill_change_log
    ADD CONSTRAINT skill_change_log_workspace_id_fkey FOREIGN KEY (workspace_id) REFERENCES public.chat_workspaces(id) ON DELETE CASCADE;


--
-- Name: skills skills_owner_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.skills
    ADD CONSTRAINT skills_owner_id_fkey FOREIGN KEY (owner_id) REFERENCES auth.users(id) ON DELETE CASCADE;
...
--
-- Name: tasks tasks_source_ref_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.tasks
    ADD CONSTRAINT tasks_source_ref_fkey FOREIGN KEY (source_ref) REFERENCES public.conversations(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY public.tasks
    ADD CONSTRAINT tasks_workspace_id_fkey FOREIGN KEY (workspace_id) REFERENCES public.chat_workspaces(id) ON DELETE CASCADE;
...
--
-- Name: skills Authenticated users can read platform and own skills; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Authenticated users can read platform and own skills" ON public.skills FOR SELECT TO authenticated USING (((owner_id IS NULL) OR (owner_id = auth.uid())));
...
--
-- Name: skills Skill admins can manage platform skills; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Skill admins can manage platform skills" ON public.skills TO authenticated USING ((COALESCE((((auth.jwt() -> 'app_metadata'::text) ->> 'role'::text) = 'admin'::text), false) OR COALESCE((((auth.jwt() -> 'app_metadata'::text) ->> 'skill_admin'::text))::boolean, false))) WITH CHECK ((COALESCE((((auth.jwt() -> 'app_metadata'::text) ->> 'role'::text) = 'admin'::text), false) OR COALESCE((((auth.jwt() -> 'app_metadata'::text) ->> 'skill_admin'::text))::boolean, false)));


--
-- Name: skill_change_log Users can append own skill change log; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can append own skill change log" ON public.skill_change_log FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = skill_change_log.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));
...
--
-- Name: canvases Users can create own canvases; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can create own canvases" ON public.canvases FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = canvases.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));
...
--
-- Name: skills Users can create own custom skills; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can create own custom skills" ON public.skills FOR INSERT TO authenticated WITH CHECK (((owner_id = auth.uid()) AND (is_custom = true)));


--
-- Name: docs Users can create own docs; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can create own docs" ON public.docs FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = docs.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));
...
--
-- Name: sheets Users can create own sheets; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can create own sheets" ON public.sheets FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = sheets.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));


--
-- Name: tasks Users can create own tasks; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can create own tasks" ON public.tasks FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = tasks.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));
...
--
-- Name: canvases Users can delete own canvases; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can delete own canvases" ON public.canvases FOR DELETE USING ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = canvases.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));
...
--
-- Name: skills Users can delete own custom skills; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can delete own custom skills" ON public.skills FOR DELETE TO authenticated USING ((owner_id = auth.uid()));


--
-- Name: docs Users can delete own docs; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can delete own docs" ON public.docs FOR DELETE USING ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = docs.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));


--
-- Name: sheets Users can delete own sheets; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can delete own sheets" ON public.sheets FOR DELETE USING ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = sheets.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));


--
-- Name: tasks Users can delete own tasks; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can delete own tasks" ON public.tasks FOR DELETE USING ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = tasks.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));
...
--
-- Name: canvases Users can read own canvases; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can read own canvases" ON public.canvases FOR SELECT USING ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = canvases.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));
...
--
-- Name: docs Users can read own docs; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can read own docs" ON public.docs FOR SELECT USING ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = docs.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));
...
--
-- Name: sheets Users can read own sheets; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can read own sheets" ON public.sheets FOR SELECT USING ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = sheets.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));


--
-- Name: skill_change_log Users can read own skill change log; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can read own skill change log" ON public.skill_change_log FOR SELECT USING ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = skill_change_log.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));
...
--
-- Name: tasks Users can read own tasks; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can read own tasks" ON public.tasks FOR SELECT USING ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = tasks.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));
...
--
-- Name: canvases Users can update own canvases; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can update own canvases" ON public.canvases FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = canvases.workspace_id) AND (chat_workspaces.user_id = auth.uid()))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = canvases.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));
...
--
-- Name: skills Users can update own custom skills; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can update own custom skills" ON public.skills FOR UPDATE TO authenticated USING ((owner_id = auth.uid())) WITH CHECK (((owner_id = auth.uid()) AND (is_custom = true)));


--
-- Name: docs Users can update own docs; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can update own docs" ON public.docs FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = docs.workspace_id) AND (chat_workspaces.user_id = auth.uid()))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = docs.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));
...
--
-- Name: sheets Users can update own sheets; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can update own sheets" ON public.sheets FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = sheets.workspace_id) AND (chat_workspaces.user_id = auth.uid()))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = sheets.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));


--
-- Name: tasks Users can update own tasks; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can update own tasks" ON public.tasks FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = tasks.workspace_id) AND (chat_workspaces.user_id = auth.uid()))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.chat_workspaces
  WHERE ((chat_workspaces.id = tasks.workspace_id) AND (chat_workspaces.user_id = auth.uid())))));


--
-- Name: canvases; Type: ROW SECURITY; Schema: public; Owner: postgres
--
...
--
-- Name: skill_change_log; Type: ROW SECURITY; Schema: public; Owner: postgres
--
...
--
-- Name: TABLE canvases; Type: ACL; Schema: public; Owner: postgres
--
...
--
-- Name: TABLE sheets; Type: ACL; Schema: public; Owner: postgres
--
...
--
-- Name: TABLE skill_change_log; Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON TABLE public.skill_change_log TO anon;
GRANT ALL ON TABLE public.skill_change_log TO authenticated;
GRANT ALL ON TABLE public.skill_change_log TO service_role;
...
\unrestrict K4gzEbHAC3U3ZPRvJTek26BpUx8TXs2FWcq78XYTy2VPOdtzeVKGtdFQZHOOhhj
