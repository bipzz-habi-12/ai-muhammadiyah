...
- **AI Providers:** OpenAI **GPT-5.6 Terra** (model utama, `OPENAI_MODEL`), Gemini 2.5, OpenRouter (fallback). **Urutan routing sejak Langkah 54: PENGGUNA yang menentukan penyedia mana dicoba lebih dulu.** Tiap model bernama (Aether/Cosmos/Prism/Velo) bisa dijalankan mesin dari Google, OpenAI, atau Anthropic — dipilih di menu model dan dikirim per pesan sebagai `modelProvider`. Aturannya:
  - **Bawaannya tetap OpenAI**, jadi pengguna yang tidak pernah memilih apa pun mengalami persis urutan lama: GPT dulu → Gemini → OpenRouter.
  - **Rantainya simetris** (Langkah 54c): apa pun yang dipilih, dua penyedia lain jadi cadangan, lalu OpenRouter. `Google → Gemini → OpenAI → Anthropic`, `OpenAI → OpenAI → Gemini → Anthropic`, `Anthropic → Anthropic → OpenAI → Gemini`. Penyedia lain tidak pernah dihapus, hanya turun peringkat.
  - Pilihan klien adalah **usulan**, bukan perintah: `resolveUsableProvider()` (`lib/ai/providers.ts`) memeriksa model itu punya mesin di sana DAN kuncinya terpasang, lalu jatuh ke OpenAI kalau tidak. Jangan pernah memindahkan keputusan ini ke klien.
  - Pengecualian pencarian web (di bawah) tetap berlaku dan **menang atas pilihan pengguna** — pesan time-sensitive tetap ke Gemini.
  - **Anthropic sudah hidup tapi BELUM TERUJI.** `streamAnthropicReply` (Langkah 54b) memanggil Messages API lewat `fetch` + `streamSseJson`, tanpa dependency baru, dan `anthropicStreamingImplemented` kini `true`. Ia belum pernah menerima permintaan sungguhan, jadi "tersedia" berarti **kunci + id model ada**, bukan terbukti menjawab — setiap kegagalan sengaja jatuh ke penyedia berikutnya. Jangan mengaktifkan extended thinking sebelum batasannya diverifikasi ke API.

  Peta model→mesin ada di `modelEngines` (`lib/subscriptions/plans.ts`) — itu sumber kebenaran untuk UI maupun server. Streaming, auto-fallback, vision, multimodal tetap seperti sebelumnya (lihat `lib/ai/chat.ts`, Langkah 35 & 54 di `MIGRATION_PROGRESS.md`).
...
- **Web search (Langkah 48, diperbaiki 2026-09-14):** `needsWebSearch()` (`lib/web-search.ts`) mendeteksi pertanyaan time-sensitive **dan** permintaan/pertanyaan kemampuan penelusuran ("bisa penelusuran web?", "cari di internet"). HANYA untuk pesan itu `streamChatReply` **melompati urutan GPT-dulu** dan memanggil Gemini dengan tool `cari_web` (panggilan bersarang `google_search`; Langkah 50). Prompt identitas **wajib bilang ya** kalau ditanya apakah M-Agent bisa mencari di web — jangan sampai model mengaku tidak punya internet. Sumber (URL redirect Google + judul = nama domain saja) dibawa lewat marker `[[AI_MU_SOURCES]]...[[/AI_MU_SOURCES]]` yang ditempel SERVER di akhir stream.
...
- **Cara memakai palet itu diperketat di Langkah 53 ("premium") — ini yang mengikat sekarang, bukan sekadar daftar warnanya:**
  ...
  - **Rantai cadangan simetris (Langkah 54c).** Apa pun penyedia yang dipilih pengguna, dua penyedia lain jadi cadangannya, lalu OpenRouter: `Google -> Gemini -> OpenAI -> Anthropic`, `OpenAI -> OpenAI -> Gemini -> Anthropic`, `Anthropic -> Anthropic -> OpenAI -> Gemini`. Pencarian web tetap dikecualikan (menuntut tool grounding Gemini).
