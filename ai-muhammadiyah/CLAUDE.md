...
- **AI Providers:** OpenAI **GPT-5.6 Terra** (model utama, `OPENAI_MODEL`), Gemini 2.5, OpenRouter (fallback). **Urutan routing sejak Langkah 54: PENGGUNA yang menentukan penyedia mana dicoba lebih dulu.** Tiap model bernama (Aether/Cosmos/Prism/Velo) bisa dijalankan mesin dari Google, OpenAI, atau Anthropic — dipilih di menu model dan dikirim per pesan sebagai `modelProvider`. Aturannya:
  ...
  - Memilih **Google** membalik urutan jadi Gemini dulu → OpenAI → OpenRouter. Penyedia lain tidak pernah dihapus, hanya turun jadi cadangan.
  ...
  - **Anthropic sengaja dilaporkan mati** (`anthropicStreamingImplemented = false`) sampai `streamAnthropicReply` ditulis, walaupun kuncinya sudah dipasang. Tanpa penjaga itu menu menawarkan Claude tapi server menjawab dengan OpenAI. Cabut flag-nya bersamaan dengan jalurnya, jangan lebih dulu.
